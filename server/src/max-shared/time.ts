...
export const isValidDate = (dateStr: Date | string) =>
  String(new Date(dateStr)) != "Invalid Date";

type DateFilter = { startDate: Date | string; endDate: Date | string };
export const getDateFilter = (
  { startDate, endDate }: Partial<DateFilter>,
  filter = {}
) => {
  if (!startDate) {
    const finDate =
      endDate && isValidDate(endDate) ? new Date(endDate) : new Date();
...
export const secondsFromNow = (seconds: number, day = new Date()) =>
  new Date(day.getTime() + seconds * 1000);
export const minutesFromNow = (minutes: number, day = new Date()) =>
  secondsFromNow(minutes * 60, day);

/**
 * YYYY-MM-DD
 */
export const dateToDashedString = (
  day = new Date(),
...
export const getMonthsBegEndPair: (period: period) => [Date, Date][] = ({
  begMonth,
  begYear,
  endMonth,
  endYear,
  ...
  while (startYear <= endYear) {
    ...
    while (state) {
      const startDate = getDate({
        year: startYear,
        month: months.indexOf(startMonth) + 1,
        day: 1,
      });
...
export const combineDateAndTime = (
  destructuredDate: DestructuredDate,
  destructuredTime: DestructuredTime
) => {
  ...
  return new Date(
    ...
    month - 1,
    day,
    timeFormat == "am" ? Number(timeHr) : toTwentyFourHrTime(Number(timeHr)),
    Number(timeMinute)
  );
};
...
export const destructureCombinedDateAndTime = (
  combinedDateAndTime: string | Date
) => {
  ...
  return {
    ...
    destructuredTime: {
      timeHr: timeHr == 0 ? "12" : `${timeHr < 10 ? `0${timeHr}` : timeHr}`,
...
export const createDateFromDestructuredDate = (
  destructuredDate: DestructuredDate
) => {
  const { day, month, year } = destructuredDate;
  return new Date(year, month - 1, day);
...
export const createTimeFromDestructuredTime = (
  destructuredTime: DestructuredTime
) => {
  const { timeHr, timeMinute, timeFormat } = destructuredTime;
  return `${timeHr}:${timeMinute}${timeFormat}`;
...
export const getDateFromTrimmedDate = (trimmedDate: string) => {
  ...
  if (trimmedDate.length > 15 || trimmedDate.length < 15)
    throw new Error("invalid date: length should be 15");
...
exports.calculateDailyHours = (from: string, to: string): number => {
  const [fromHours, fromMinutes] = convertTo24HourFormat(from)
    .split(":")
    .map(Number);
